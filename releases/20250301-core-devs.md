...
12. Move OUI/Devaddr Managment On Chain
...
Move the managment of OUI and Devaddr to on chain

#### Motivation

Currently OUI/Devaddr management is done via a centralized database with no structures on chain. This will move the two structures on chain and allow anyone to request creation of an oui. Once an oui is created, the owner can create as many devaddrs as they are willing to pay the fee for.

#### Implementation

A new smart contract named iot-routing-manager will be deployed to facilitate the endpoints required for the on chain managment of OUI/Devaddrs. This will also include 3 temporary endpoints that will be used to backfill the existing data and recreate the current state of things on chain.

All refrences for OUI/Devadder throughout [oracles](https://github.com/helium/oracles) will be updated to work with the new structures/endpoints.

All cli commands in [helium-config-service-cli](https://github.com/helium/helium-config-service-cli) will be updated to work with the new structures/endpoints.

#### Impact

Users will have better control over creating and updating OUI devaddrs and delegates.
